...
class BaileysAPI:
    ...
    def get_status(self, chat_id_usuario: Optional[int] = None, session: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            # O servidor não envia 'success' aqui; tratamos como sucesso se vier chaves esperadas.
    ...
    def generate_qr_code(self, chat_id_usuario: Optional[int] = None, session: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            resp = self._make_request(endpoint, 'GET')
            if (isinstance(resp, dict) and (resp.get('success') or ('qr' in resp or 'qr_image' in resp))) or \
               (isinstance(resp, dict) and 'data' in resp and ('qr' in resp['data'] or 'qr_image' in resp['data'])):
